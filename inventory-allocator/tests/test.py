...
def test_factory_consolidation():
    ...
    assert(checkEqual(expected,output))
    print('test_factory_consolidation passed!')
...
def test_ship_one_factory():
    ...
    warehouses =[
        ...
        { 'name': 'bd', 'inventory': { 'orange': 5} },
        { 'name': 'all', 'inventory': { 'apple': 5, 'banana':5, 'orange': 5} }
    ]
    output = inst.calculateShipment(item_map, warehouses)
    expected = [{'all': {'apple': 5, 'banana': 5, 'orange': 5}}]
    assert(checkEqual(expected,output))
    print('test_ship_one_factory passed!')
